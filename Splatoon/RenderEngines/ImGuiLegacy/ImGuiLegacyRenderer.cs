            ...
            if(element.refActorType == 1 && LayoutUtils.CheckCharacterAttributes(element, Svc.Objects.LocalPlayer, true))
            {
                ...
                if(element.type == 1)
                {
                    ...
                    DrawCircle(layout, element, pointPos.X, pointPos.Y, pointPos.Z, radius, element.includeRotation ? Svc.Objects.LocalPlayer.GetRotationWithOverride(element) : 0f,
                        element.overlayPlaceholders ? Svc.Objects.LocalPlayer : null);
                }
                else if(element.type == 3)
                {
                    AddRotatedLine(layout, Utils.GetPlayerPositionXZY(), Svc.Objects.LocalPlayer.GetRotationWithOverride(element), element, radius, 0f);
                ...
                else if(element.type == 4)
                {
                    if(element.coneAngleMax > element.coneAngleMin)
                    {
                        ...
                        {
                            AddConeLine(Utils.GetPlayerPositionXZY(), Svc.Objects.LocalPlayer.GetRotationWithOverride(element), (Svc.Objects.LocalPlayer.GetRotationWithOverride(element).RadiansToDegrees() - x.Float()).DegreesToRadians(), element, radius, x == element.coneAngleMin ? 1f : element.fillIntensity ?? Utils.DefaultFillIntensity, false);
                        }
                        AddConeLine(Utils.GetPlayerPositionXZY(), Svc.Objects.LocalPlayer.GetRotationWithOverride(element), (Svc.Objects.LocalPlayer.GetRotationWithOverride(element).RadiansToDegrees() - element.coneAngleMax.Float()).DegreesToRadians(), element, radius, 1f, true);
                    }
                }
            }
