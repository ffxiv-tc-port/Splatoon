...
public class GenericTargetEnforcer : SplatoonScript
{
    ...
    public override void OnUpdate()
    {
        ...
        {
            if(!values.Contains(Svc.Targets.Target?.BaseId ?? 0))
            {
                var obj = Svc.Objects.OrderBy(Player.DistanceTo).FirstOrDefault(x => values.Contains(x.BaseId) && x.IsTargetable && !x.IsDead);
