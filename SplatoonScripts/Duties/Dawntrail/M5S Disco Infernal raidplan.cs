...
public sealed class M5S_Disco_Infernal_RaidPlan : SplatoonScript
{
    ...
    {
        ImGuiEx.CollectionCheckbox("EastInside", Direction.EastInside, C.Directions);
        ImGuiEx.CollectionCheckbox("NorthEastOutside", Direction.NorthEastOutside, C.Directions);
        ImGuiEx.CollectionCheckbox("NorthInside", Direction.NorthInside, C.Directions);
        ImGuiEx.CollectionCheckbox("NorthWestOutside", Direction.NorthWestOutside, C.Directions);
        ImGuiEx.CollectionCheckbox("SouthInside", Direction.SouthInside, C.Directions);
        ImGuiEx.CollectionCheckbox("SouthEastOutside", Direction.SouthEastOutside, C.Directions);
        ImGuiEx.CollectionCheckbox("WestInside", Direction.WestInside, C.Directions);
        ...
        private void SetBait(bool isShortDebuff)
    {
        if (_isSafeOutSideNorthWest)
        {
            ...
            else if (C.Directions.Contains(Direction.EastInside))
            {
                var original = HasSpotLightAt(3, 5) ? (3, 5) : (5, 3);
                var final = isShortDebuff ? (original == (3, 5) ? (5, 3) : (3, 5)) : original;
                SetElementFromIndex(final.Item1, final.Item2);
            }
            else if (C.Directions.Contains(Direction.WestInside))
            {
                var original = HasSpotLightAt(2, 4) ? (2, 4) : (4, 2);
                var final = isShortDebuff ? (original == (2, 4) ? (4, 2) : (2, 4)) : original;
                SetElementFromIndex(final.Item1, final.Item2);
            }
        }
        else
        {
            ...
            else if (C.Directions.Contains(Direction.NorthInside))
            {
                var original = HasSpotLightAt(3, 2) ? (3, 2) : (2, 3);
                var final = isShortDebuff ? (original == (3, 2) ? (2, 3) : (3, 2)) : original;
                SetElementFromIndex(final.Item1, final.Item2);
            }
            else if (C.Directions.Contains(Direction.SouthInside))
            {
    ...
    private class Config : IEzConfig
    {
        ...
        [
            Direction.NorthInside,
            Direction.WestInside
        ];
    }
}
