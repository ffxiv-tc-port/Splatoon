...
internal unsafe class Hello_World_MoveGuide : SplatoonScript
{
    ...
    public class Config : IEzConfig
    {
        ...
        public Vector3 refOffset = new(0, 0, 0);
    }
    ...
    private void Proc()
    {
        ...
        if(state == State.PartyInitDone)
        {
            ...
            var CircleRot = HasColorRot.None;
            foreach(var x in partyList)
            {
    ...
    private void GuideCircle()
    {
        switch(state)
        {
            case State.LatentDefectCasting:
                var searchID = (myData.hasColorRot == HasColorRot.RedRot) ? CastIDs.RedRot : CastIDs.BlueRot;
                var towerList = Svc.Objects.OfType<IBattleChara>().Where(x => x.CastActionId == searchID).ToList();
            ...
            case State.WaitingRot:
                var nearestWaymark = GetNearestWaymark();
    ...
    private void GuideShare()
    {
        ...
        {
            case State.LatentDefectCasting:
                var myTower = DecideTower(towerList, partyList.Where(x => x.gimmickRole == GimmickRole.Share).Select(x => x.pcPtr).ToList());
                var otherTower = towerList.Where(x => x.EntityId != myTower.EntityId).First();
                var firstLeftRight = GetRelativePosition(myTower.Position, CenterPos, otherTower.Position);
        ...
        if(towerList.Count != 4)
        {
            switch(state)
            {
                case State.LatentDefectCasting:
                    ...
                    var myTower = DecideTower(towerList, partyList.Where(x => x.gimmickRole == GimmickRole.Share).Select(x => x.pcPtr).ToList());
                    var firstLeftRight = GetRelativePosition(towerList.First().Position, CenterPos, towerList.Last().Position);
                    if(firstLeftRight == "right")
                    {
    ...
    private void GuideReceiveShare()
    {
        ...
        if(latentCount != 4)
        {
            switch(state)
            {
                ...
                case State.WaitingRot:
                case State.Waiting:
                    var midPoint = CalculateMidpoint(firstTowerPos, secondTowerPos);
                    ShowGuidePointFixPos(OffsetTowardsNorth(midPoint, CenterPos, new(0, 0, 4)));
                    break;
            }
        }
    ...
    private IBattleChara DecideTower(List<IBattleChara> towerList, List<IPlayerCharacter> playerRoleList)
    {
        ...
        // Calculate distances
        var myDistanceToTower1 = Vector3.Distance(myData.pcPtr.Position, towerList.First().Position);
        var myDistanceToTower2 = Vector3.Distance(myData.pcPtr.Position, towerList.Last().Position);
        var pairDistanceToTower1 = Vector3.Distance(pair.Position, towerList.First().Position);
        var pairDistanceToTower2 = Vector3.Distance(pair.Position, towerList.Last().Position);
        ...
        var tower1IDHex = Convert.ToString(towerList.First().EntityId, 16);
        var tower2IDHex = Convert.ToString(towerList.Last().EntityId, 16);
    ...
    public static Vector3 OffsetTowardsNorth(Vector3 pos, Vector3 targetPos, Vector3 offset)
    {
        ...
        var northDirection = Vector3.Normalize(targetPos - pos);

        // Offset based on the "north" direction
        // Calculate the right direction relative to the north direction
        var rightDirection = Vector3.Cross(Vector3.UnitY, northDirection);
        var forwardDirection = northDirection;
    ...
    public Vector3 GetNearestWaymark(Vector3? referencePos = null)
    {
        ...
        {
            var markerPos = mkc->FieldMarkers[i].Position;

            var distance = Vector3.Distance(basePos, markerPos);
    ...
    public string GetRelativePosition(Vector3 currentPos, Vector3 referencePos, Vector3 targetPos)
    {
        ...
        var perspectiveDirection = currentPos - referencePos;
        ...
        var targetDirection = targetPos - referencePos;
        ...
        var crossProduct = (perspectiveDirection.X * targetDirection.Z) - (perspectiveDirection.Z * targetDirection.X);
